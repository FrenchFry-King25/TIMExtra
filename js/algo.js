...
function writeTasks(user) {
  ...
  for (const task in tasks) {
    ...
    if ((tasks[task].start == null || tasks[task].end == null) && array.length != 0) {
      let start = array.slice(-1).end + breaktime
      console.log(start)
      if (start % 100 >= 60) {
        start += 40
      }
...
function makeCalendar(user) {
  ...
  headingDiv.setAttribute('class', 'flex flex-row justify-between border-solid border-4 w-max')
